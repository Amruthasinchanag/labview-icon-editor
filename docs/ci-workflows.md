...
- **powershell-lint** – runs PSScriptAnalyzer against repository PowerShell scripts using the shared ruleset and baseline (runs on GitHub-hosted `ubuntu-latest`, so it does not consume self-hosted runner time).
...
3. **Label the Runner**:
   ...
   - Example labels: `self-hosted-windows-lv`, `self-hosted-windows-lv-ie`.
